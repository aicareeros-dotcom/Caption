...
BAD_WORDS = ["Free", "@ErAnkitEh", "@EhcoderGec", "⌯ 𝚬 𝚪 𝚼 𝚾 · ༎🖤", " ⌯ ꪀꪖᴠꫀꫀꪀ·༎🖤", "𝙎𝘼𝙄𝙉𝙄 𝘽𝙊𝙏𝙎🐦", "𝐈𝗆𝗉𝗋𝖾𝗌𝗌𝗂𝗏𝖾❤️"]
...
async def queue_worker(client, user_id):
    while True:
        ...
        try:
            ...
            caption = process_text(msg.caption or "")
...
async def set_channel(client, message):
    await message.reply("📢 Channel ID bhejo (example: -100xxxxxxxxxx)")

    try:
        msg = await client.listen(message.chat.id, timeout=60)
        ...
        try:
            ...
            await message.reply("❌ Bot is not admin in channel")
            return

        user_channels[message.from_user.id] = ch_id
        await message.reply(f"✅ Channel Saved:\n`{ch_id}`")
...
async def handle_media(client, message: Message):
    ...
    if user_id not in user_queues:
        ...
        asyncio.create_task(queue_worker(app, user_id))
